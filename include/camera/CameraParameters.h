...
#if !defined(THE_WRAPPER) && !defined(CAMERA_SOURCE_H_) && !defined(STAGEFRIGHT_RECORDER_H_) && !defined(HW_EMULATOR_CAMERA_EMULATED_CAMERA_H)
// Hacky hack: Define a symbol used by the GB blob here...
...
/* And now the real ugliness begins: Let's stick the overlay symbols
 * into libcamera_client so that they're available to libcamera, and
 * make the Overlay class definition available */

/* --------------- Originally Overlay.h --------------- */
...
/* Exclude all the headers that refer to CameraParams.h. We only want these
 * symbols available in the libcamera client */

/* --------------- Originally Overlay.cpp --------------- */
#if !defined(THE_WRAPPER) && !defined(CAMERA_SOURCE_H_) && !defined(STAGEFRIGHT_RECORDER_H_) && !defined(HW_EMULATOR_CAMERA_EMULATED_CAMERA_H)
...
#endif // THE_WRAPPER+friends exclusion
