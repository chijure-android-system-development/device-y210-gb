...
## Sesión 2026-07-12: captura en vivo dual (stock + CM7 conectados a la vez) -- cierra la hipótesis de timing/orden pendiente de cont.10

Con ambos equipos conectados por USB simultáneamente, se limpiaron `dmesg`/`logcat` en ambos, se sintonizó 98.1 MHz en cada uno por separado, y se volcó todo con timestamps (dmesg del kernel trae UTC real embebido vía el driver de suspend, usado como ancla para correlacionar con logcat). Esto llena el hueco que cont.8 había dejado explícitamente pendiente ("capturar logcat verboso... en el STOCK... no se ha hecho").

**Hallazgo 1 -- confirmado con logcat real del stock (antes no se tenía):** el stock corre una app de FM propia de Huawei (`com.huawei.android.FMRadio` / servicio `FMService2`), no la app CAF de referencia (`com.android.fm`) que trae CM7. Su `AudioPolicyManager` (tag propietario, distinto del genérico `AudioPolicyManagerBase` de AOSP) registra `"FM started in 1 Mode"` una sola vez y, en un reintento de conexión posterior, `"FM is already connected in 1 Mode"` -- es decir, **el stock nunca hace un ciclo real UNAVAILABLE→AVAILABLE** como el "bounce" de `switchToSpeaker()/switchToHeadset()` que sí ejecuta la app CAF en CM7. Esto es consistente con -- no contradice -- la conclusión ya cerrada en cont.8 (el bounce es necesario en CM7 con la arquitectura actual, quitarlo da silencio total): el stock simplemente nunca necesita evaluarlo porque su app nunca duplica la llamada de conexión.

**Hallazgo 2 -- descartado como pista, es ruido genérico de AOSP:** en CM7, incluso en la primerísima llamada a `doRouting()` (antes del bounce), el ruteo pasa por dos pasadas en ~20ms: primero calcula "Speaker (dev=27)" con el warning `"Hardware does not support requested route combination"`, y se corrige a "Headset (dev=26)" en la siguiente pasada -- cada una disparando su propio `BTFMPinSwitching`. Rastreado hasta `AudioPolicyManagerBase::setDeviceConnectionState()` (`frameworks/base/services/audioflinger/AudioPolicyManagerBase.cpp:141-151`): `updateDeviceForStrategy()` (línea 150) recalcula y aplica `setOutputDevice()` con un estado de dispositivos aún no actualizado *antes* de que el `setOutputDevice(mHardwareOutput, newDevice)` correcto de la línea 151 se aplique con el valor ya correcto. Es lógica genérica de AOSP que corre para **cualquier** evento de conexión de dispositivo (no específica de FM, no introducida por ningún fix nuestro) y se resuelve en ~20ms, muy por debajo de cualquier escala de tiempo relevante para el ruido sostenido e independiente de la frecuencia. **Descartado como causa.**

**Balance:** la vía de "timing/orden" que cont.10 dejó como pendiente de mayor esfuerzo se investigó con evidencia en vivo de ambos lados y no aportó una causa nueva -- solo confirmó (con más detalle) conclusiones ya cerradas. El techo real sigue siendo el mismo de cont.8/10/11: configuración del DSP/ARM9 vía RPC opaca, solo investigable con un analizador lógico en las líneas I2S físicas o símbolos de depuración del firmware AMSS.

**Pista nueva sin probar aún -- BT SCO/HFP como comparación directa de hardware compartido (2026-07-12):** el WCN2243 es un chip combo BT+FM, y `switch_mode()`/`BTFMPinSwitching` (código fuente real confirmado en cont.2, 2026-07-09) prueba que `MODE_FM` y `MODE_BTSCO` literalmente comparten los mismos pines físicos I2S/PCM del chip (tristatean uno para activar el otro). Bluetooth en este port solo se validó con **A2DP** (`FUNCTION_MATRIX.md`/`BLUETOOTH_NOTES.md`, 2026-07-10) -- audio codificado SBC que viaja como datos por el enlace de radio y se decodifica en el auricular remoto, **sin tocar nunca** el DAC/I2S interno del chip ni los pines compartidos con FM. SCO/HFP (audio de llamada de voz por auricular BT) sí usaría esos mismos pines/mux -- **nunca se ha probado en este port**. Es la comparación de hardware compartido más directa disponible sin instrumentación:
- Si una llamada real por SCO/HFP también sale con ruido/silencio → apunta a un problema genérico en los pines compartidos o en el códec MSM7x27a, no específico del núcleo FM.
- Si SCO/HFP suena limpio → descarta el hardware/pines compartidos como causa, y reenfoca la sospecha en algo específico de cómo el núcleo FM (vs. el núcleo BT) configura esos mismos pines (rol maestro/esclavo del reloj, formato, timing de inicialización) -- reabriría la hipótesis de "timing/orden" con un ángulo nuevo y concreto (comparar la secuencia de configuración de pines que usa el núcleo BT-SCO, que sí funciona, contra la que usa hoy el núcleo FM).

## Sesión 2026-07-12 (cont.): RESUELTO -- audmgr_enable() era la pieza faltante, mal descartada en 2026-07-06 por un bug ya corregido

Revisando una comparación externa del historial de esta investigación, se notó que la prueba que "descartó definitivamente" `audmgr_enable()`/`/dev/msm_fm` (sesión 2026-07-06, cont.6) usó `switch_mode(1)` creyendo que era `MODE_FM` -- pero el bug de etiquetas invertidas de `switch_mode` no se encontró hasta el 2026-07-09 (cont.2): `MODE_FM=0`, `MODE_BTSCO=1`. Es decir, esa prueba de 2026-07-06 corrió con los pines I2S de FM **tristateados** (en modo BT), no en modo FM real -- el resultado "peor, silencio total" se explica completamente por eso, sin decir nada sobre si `audmgr_enable()` en sí es compatible con una ruta FM I2S real. Nunca fue una prueba limpia.

**Se reimplementó `run_fm_audmgr_session()`** (`AudioHardware.cpp`): abre `/dev/msm_fm`, `ioctl(AUDIO_START)` en `fmOn`/`ioctl(AUDIO_STOP)`+close en `fmOff`, con el fd persistente en `fmfd` (miembro ya existente, sin usar desde el revert de 2026-07-06). Llamado desde `setFmOnOff()` **antes** de que `doAudioRouteOrMute()` haga el RPC -- replicando el orden exacto visto en el dmesg del stock real (`audmgr_enable() → RPC READY → RPC CODEC_CONFIG → rpc_snd_set_device`). El resto de la cadena (switch_mode con las etiquetas ya correctas desde 2026-07-09, `FmAudioPathEnable` con los registros del codec ya igualados, `config_dac`) se dejó sin tocar.

**Resultado en hardware real: AUDIO REAL, ESTABLE.** Confirmado por el usuario escuchando 98.1 MHz: contenido reconocible (música/voz de la emisora real), sin cortes, sin volver a ruido. Logcat confirma la secuencia limpia en cada ciclo on/off: `run_fm_audmgr_session: AUDIO_START succeeded` → `Routing FM audio to Headset/Speaker` → `BTFMPinSwitching: switch mode(0) succeeded` → `FmAudioPathEnable: succeeded`, con `RSSI check @ 98100: immediate=100` sostenido.

**Causa raíz real, con retrospectiva de toda la investigación:** todo lo que se investigó a fondo (registros Marimba, GPIOs I2S, ID de dispositivo, firmware/calibración del WCN2243, switch_mode/pin-switching, coexistencia BT/FM) efectivamente coincidía con el stock -- porque **ninguno de esos era la causa**. Sin la sesión `audmgr` registrada con el ARM9, el AP puede configurar el I2S/RPC/pin-switch perfectamente (de ahí que todo lo inspeccionable desde Linux coincidiera con stock) pero **el DSP/ARM9 nunca recibe la señal de que debe habilitar/enrutar la sesión de audio real** -- de ahí el ruido de piso (líneas I2S activas pero sin una sesión de audio real detrás) en vez de silencio total (cuando además los pines estaban mal, en las pruebas donde se combinó con el bug de switch_mode). El techo de "config DSP/ARM9 vía RPC opaca desde Linux" que se documentó como límite real de la investigación (cont.8/10/11) era correcto en el diagnóstico pero incompleto en la solución: la señal que faltaba SÍ es controlable desde Linux -- es la llamada a `audmgr_enable()` vía `/dev/msm_fm`, no algo dentro del propio RPC de `snd_set_device`.

**Pendiente de validar (no bloqueante, calidad/robustez):** ciclos de encendido/apagado repetidos (ya se ven varios en el log de hoy, todos limpios), modo altavoz vs auricular (ambos aparecen en el log de hoy, pendiente confirmación auditiva explícita de altavoz), persistencia tras reboot completo (el fix está en el árbol fuente, así que un build limpio ya lo incluye), y la prueba SCO/HFP que había quedado pendiente ya no es necesaria para resolver el bug de audio (aunque sigue siendo válida como dato de coexistencia BT/FM si se quiere profundizar).

Pendiente de auricular/altavoz Bluetooth con micrófono disponible para probar SCO/HFP (la comparación fuerte).

## Sesión 2026-07-12 (cont. 3): bug de volumen encontrado tras el fix de audio, y limpieza de 6 bugs reales sin relación con FM

Con el audio ya funcionando, el usuario encontró dos problemas reales en `setFmVolume()`:

1. **No muteaba nunca de verdad al bajar el volumen al mínimo** -- la versión original forzaba un piso de `vol=1` ("keep minimum at 1 so the path is never muted when the user deliberately enables FM"), pero ese caso ya está cubierto por separado por el `ear_mute` de `doAudioRouteOrMute()`. Corregido para permitir `vol=0` real.
2. **Los pasos bajos e intermedios del slider no se sentían** -- `STREAM_FM` tiene 15 pasos en la UI (`AudioService.java`), pero `AudioPolicyManagerBase::computeVolume()` aplica una curva logarítmica (`AudioSystem::linearToLog()`) pensada para streams con más resolución que nuestros 6 niveles RPC. Mapear el valor log directo a `[0,5]` amontonaba ~10 de los 15 pasos en el mismo nivel. Fix: usar `AudioSystem::logToLinear()` (la inversa exacta) para recuperar el porcentaje lineal 0-100 antes de cuantizar, repartiendo los 6 niveles de forma pareja.

Versión final adoptada (mejor que la primera iteración): usa aritmética entera (`(volPct+19)/20`, sin floats), y llama `set_volume_rpc(SND_DEVICE_CURRENT, ...)` una sola vez -- el mismo patrón que ya usa `setVoiceVolume()` en este archivo -- en vez de fijar volumen por separado en `fmHst`/`fmSpk`. Revisa y propaga el resultado del RPC (la versión anterior lo descartaba en silencio).

**Además, se encontraron y corrigieron 6 bugs reales sin relación con el bug de audio de FM** (verificados uno por uno contra el código, no aplicados a ciegas -- varios de una lista externa de 22 "bugs" resultaron ser falsos positivos al leer el código completo, ver detalle si se necesita en el historial de la sesión):
- `get_audpp_filter()`: el registro NS (`'G'`) hacía `strtok(NULL, seps)` sin reiniciar el tokenizador con `strtok(buf, ",")` como sí hacen los registros `'E'`/`'F'` -- heredaba el estado de la línea anterior, rompiendo el parseo de cualquier línea NS del CSV.
- `AudioStreamInMSM72xx::mFirstread` no estaba en la lista de inicialización del constructor -- valor indeterminado en la primera lectura tras construir el objeto.
- `mTtyMode` se reseteaba a `TTY_OFF` en el `else` de cualquier `setParameters()` que NO incluyera la clave `tty_mode` -- es decir, cualquier cambio de audio no relacionado (FM, ruteo, BT) apagaba silenciosamente el modo TTY durante una llamada.
- `mTtyMode`, `mNumSndEndpoints`, `audpre_index` no se inicializaban en el constructor de `AudioHardware`.
- `SND_GET_ENDPOINT` no comprobaba el resultado del ioctl ni limpiaba la estructura antes -- un fallo dejaba `ept->name` con contenido indeterminado antes del `strcmp`.
- `doRouting()` desreferenciaba `mOutput->devices()` sin comprobar `mOutput != NULL`.

Quedaron sin aplicar (confirmados como reales pero de menor severidad o que tocan la ruta FM ya estabilizada, ver conversación para el detalle completo si se retoman): los 8 relacionados con el orden RPC/pin-switch/postproc de FM (ninguno explica el bug de audio ya resuelto por `run_fm_audmgr_session`), el bug del destructor que salta elementos de `mInputs` (impacto práctico nulo, el proceso se cierra igual), y algunos sin verificar por tiempo (parser AAC, bounds del CSV, `standby()` de entrada, loop de `write()==0`).

**Control rápido ya hecho (2026-07-12): FM con Bluetooth completamente apagado -- sin cambio, descarta regulador/hardware compartido como causa por sí solo.** Confirmado con dmesg que BT se apagó de verdad (`hci0` desaparece de `hciconfig`, `Bluetooth: HCI_IBS stats: vote ticks: on=1192, off=455058` justo antes de la prueba). Con BT off, el patrón `snd_set_device`/`tavarua_radio` al encender FM es idéntico al de siempre (mismo baile 26→3→26) y el resultado audible es el mismo (estática, sin cambio). Esto descarta que el simple estado ON/OFF de BT (o la configuración de reguladores `internal_vreg_ctl[bt_status]` que el driver ajusta según ese estado) sea la causa -- pero **no** descarta todavía la hipótesis más fuerte (SCO/HFP activo compitiendo en tiempo real por los mismos pines), que sigue pendiente de auricular BT con micrófono.
