...
/* Suppress the embedded Overlay class hack in the device tree's
 * <camera/CameraParameters.h> (only needed by the old blob-wrapper build) —
 * we link libcamera_client normally and get the real android::Overlay from
 * there, so the duplicate would conflict at link time. */
#define THE_WRAPPER 1
