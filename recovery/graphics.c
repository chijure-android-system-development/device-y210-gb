...
/* panel does not like double buffering; force single buffer */
static int gr_single_buffer = 1;
...
void gr_flip(void)
{
     ...
     * to make active. Skip copy when drawing directly on fb. */
    if (!gr_single_buffer) {
...
int gr_init(void)
{
    ...
    if (gr_single_buffer) {
        /* draw directly on the visible buffer */
        gr_mem_surface.data = gr_framebuffer[0].data;
        gr_mem_surface.stride = gr_framebuffer[0].stride;
    }
    ...
    /* Debug fill to verify panel output: colored stripes */
    {
        unsigned short *p = (unsigned short *)gr_mem_surface.data;
        int stride = gr_mem_surface.stride;
        int h = gr_mem_surface.height;
        int y;
        for (y = 0; y < h; ++y) {
            unsigned short color = (y < h / 3) ? 0xF800 /* red */ :
